...
`--history-context-pooling mean` masked-mean-pools the strictly-prior short
history; `attention` uses the scenario embedding as a query to weight those
items before projection into the reasoning prompt. Both are auditable and
leakage-safe, but neither is the paper's Slow-LLM-generated textual instruction.
