...
- 环境前提：本次NPU运行曾因TorchNPU后端动态库未正确加载而失败，补充`/usr/lib64`并依次加载
  driver与CANN环境后成功；
- 未验证：固定验证指标对齐、正式显存/吞吐和多卡。

当前标记为`[v2 Full FP32/BF16迁移短训练通过]`。独立设备训练使用各自随机数实现，输出checkpoint
哈希不要求相同；只要求起始代码和输入一致、训练量有限且保存恢复成功。固定验证指标通过前
不称为最终精度对齐；固定验证入口已实现但尚待两台服务器实跑。CPU fallback未确认前不进入性能调优。
