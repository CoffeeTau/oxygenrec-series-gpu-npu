...
- 当前状态：Stage-0已在8×Ascend 950DT服务器通过；v2 Full固定参考已在NVIDIA L20生成，NPU精度比较待执行；
...
下一步把已冻结的GPU参考包原样传到NPU，在NPU比较
logits/loss/greedy/beam、全量梯度与单步参数delta；在该阶段通过前，不把GPU单侧成功
